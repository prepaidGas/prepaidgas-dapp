...
contract GasOrder is IGasOrder, FeeProcessor, ERC1155ish {
    ...
    modifier executionCallback() {
        if (execution != msg.sender)
            revert Error.Unauthorized(msg.sender, execution);
        _;
    }
  ...
  modifier orderExists(uint256 id) {
    ...
    modifier deadlineNotMet(uint256 deadline) {
        if (deadline <= block.timestamp)
            revert Error.DeadlineExpired(block.timestamp, deadline);
        _;
    }

    modifier possibleExecutionWindow(uint256 window) {
        if (window < Const.MIN_EXEC_WINDOW)
            revert Error.OverlowValue(window, Const.MIN_EXEC_WINDOW);
        _;
    }
    ...
    {
        require(
            executionPeriodStart + executionWindow < executionPeriodDeadline
        ); // @todo add error msg
    ...
    function acceptOrder(
        uint256 id,
        uint256 guaranteeTransfer
    ) external specificStatus(id, OrderStatus.Pending) {
        ...
        _distribute(
            msg.sender,
            reward[id].token,
            _takeFee(reward[id].token, reward[id].amount)
        );
        _acceptIncoming(
            guarantee[id].token,
            msg.sender,
            guaranteeTransfer,
            totalSupply(id) * guarantee[id].gasPrice
        );
    ...
    function retrieveGasCost(
        ...
        if (
            ...
            _distribute(
                executor[id],
                guarantee[id].token,
                guarantee[id].gasPrice * amount
            );

        IERC20(gasCost[id].token).safeTransfer(
            holder,
            gasCost[id].gasPrice * amount
        );
    }
     ...
     */
    function retrieveGuarantee(
        uint256 id
    ) external specificStatus(id, OrderStatus.Inactive) {
        _guaranteeAndRewardDelivered(id); // @todo it might be already applied
        _distribute(
            executor[id],
            guarantee[id].token,
            guarantee[id].gasPrice * totalSupply(id)
        );
    }
    ...
    function revokeOrder(uint256 id) external {
        ...
        if (msg.sender != currentOrder.creator)
            revert Error.Unauthorized(msg.sender, currentOrder.creator);

        OrderStatus currentStatus = status(id);
        if (
            currentStatus == OrderStatus.Pending ||
            (currentOrder.isRevokable &&
                (currentStatus == OrderStatus.Accepted ||
                    currentStatus == OrderStatus.Active))
        ) {
            _guaranteeAndRewardDelivered(id);
            IERC20(reward[id].token).safeTransfer(
                order[id].creator,
                reward[id].amount
            );
            ...
            IERC20(gasCost[id].token).safeTransfer(
                order[id].creator,
                gasCost[id].gasPrice * order[id].maxGas
            );
        } else {
            revert Error.RevokeNotAllowed(
                currentOrder.isRevokable,
                currentStatus
            );
        }
    ...
    function reportExecution(
        ...
        if (gasLimit > balance)
            revert Error.GasLimitExceedBalance(gasLimit, balance);
        ...
        if (fulfiller == executor[id]) {
            ...
            _distribute(
                fulfiller,
                unlockToken,
                _takeFee(unlockToken, unlockAmount)
            );
        }
    }
    ...
    function status(uint256 id) public view returns (OrderStatus) {
        ...
        if (
            ...
            order[id].executionPeriodDeadline >= block.timestamp
        ) {
            return OrderStatus.Pending;
        }

        if (executor[id] != address(0) && totalSupply(id) == 0)
            return OrderStatus.Inactive;

        if (order[id].executionPeriodStart > block.timestamp)
    ...
    function getFilteredOrders(
        ...
        uint256 limit = (_limit > Const.MAX_FILTERED_ORDERS)
            ? Const.MAX_FILTERED_ORDERS
            : _limit;
        ...
        for (
            uint256 orderId = startIndex;
            orderId < ordersCount && addedOrders < limit;
            orderId++
        ) {
            if (
                (_creator == address(0) ||
                    order[orderId].creator == _creator) &&
                ...
                result[addedOrders] = FilteredOrder({
                    ...
                    executionPeriodDeadline: order[orderId]
                    ...
                    availableGasHoldings: _user != address(0)
                        ? balanceOf(_user, orderId)
                        : 0 // @todo implemen `_user` calculations
    ...
    function totalMatchingOrdersCount(
        address _creator,
        OrderStatus _status
